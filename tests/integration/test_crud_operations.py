...
async def test_full_crud_lifecycle(registered_engine, test_db):
    ...
    result = await crud.list(
        filters={"email": "integration@test.com"},
        per_page=10
    )
...
async def test_pagination_consistency(registered_engine, test_db):
    ...
    products = [
        {"name": f"Product {i:02d}", "price": i * 10, "stock": 100}
        for i in range(25)
    ]
...
async def test_bulk_operations(registered_engine, test_db):
    ...
    test_users = [
        {"name": f"Bulk User {i}", "email": f"bulk{i}@test.com", "age": 20 + i}
        for i in range(5)
    ]
...
async def test_filtering_with_operators(registered_engine, test_db):
    ...
    results = await crud.list(filters={
        "category": "books",
        "price__gte": 30
    })
    assert results["total"] >= 2
