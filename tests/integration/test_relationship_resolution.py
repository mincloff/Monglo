...
async def test_relationship_detection_naming_convention(test_db):
    ...
    await test_db.posts.insert_one({
        "user_id": user_id,  # Should auto-detect as relationship
        "title": "Test Post"
    })
...
async def test_resolve_one_to_many_relationship(test_db):
    ...
    await test_db.posts.insert_one({
        "title": "My Post",
        "tag_ids": tag_ids  # Array of ObjectIds
    })
...
async def test_relationship_batch_resolution(test_db):
    ...
    posts = [
        {"title": f"Post {i}", "user_id": user_id}
        for i in range(10)
    ]
    ...
    # Batch resolve
    resolver = RelationshipResolver(test_db)
    resolved_posts = await resolver.resolve_batch(
        all_posts,
        posts_admin.relationships,
        depth=1
    )
...
async def test_bidirectional_relationships(test_db):
    ...
    await engine.register_collection("posts", config=CollectionConfig(
        relationships=[
            Relationship(
